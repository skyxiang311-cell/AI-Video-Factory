...
export const CHAPTER_DEEP_READ_PROMPT_VERSION = "claim-first-chapter-v1";
...
): {analysis: ChapterAnalysis; blockingIssues: string[]} => {
...
const readReusable = async (
  ...
  try {
    ...
    return validated.blockingIssues.length === 0 ? validated.analysis : null;
  } catch {
    return null;
  }
};
...
export const createOrReuseTargetChapterAnalyses = async ({
  ...
  for (const chapterId of targetIds) {
    ...
    const validated = normalizeAndValidate(
      source,
      input,
      ChapterAnalysisSchema.parse(await provider.analyzeChapter(input)),
    );
    const candidateIssues = [
      ...validated.blockingIssues,
      ...repeatedClaimIssues([...analyses, validated.analysis]),
    ];
    if (candidateIssues.length > 0) {
      throw new Error(`Chapter analysis validation failed: ${candidateIssues.join("; ")}`);
    }
  ...
  const setIssues = validateChapterAnalysisSet(analyses);
  if (setIssues.length > 0) {
    throw new Error(`Chapter analysis set validation failed: ${setIssues.join("; ")}`);
  }
  return {analyses, cacheHits, blockingTraceabilityIssues: []};
};
